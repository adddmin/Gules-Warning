...
class ashuwp_framework_core {
  ...
  public function enqueue_css_js() {
    wp_enqueue_media();
    wp_enqueue_style('framework_css', get_template_directory_uri(). '/includes/metabox/css/framework.css');
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_script( 'wp-color-picker' );
    wp_enqueue_script( 'jquery-ui-tabs' );
    wp_enqueue_script('framework_js', get_template_directory_uri(). '/includes/metabox/js/framework.js','','',true);
    wp_localize_script( 'framework_js', 'ashu_file_preview', array('img_base'=>includes_url(),'img_path'=>$this->file_ico,'ajaxurl' => admin_url( 'admin-ajax.php' )));
  }
...
/*当前文章所属专题*/
public function belong_topic($values) {
    global $wpdb;
    //获取连接中 post 后面的参数
    $get_post = $_GET['post'];
    ...
    echo $values['desc'];
    echo '<ol>';
    $args=array( 'post_type'=> 'topic','ignore_sticky_posts' => 1 );$wp_query = new WP_Query( $args );if ( $wp_query->have_posts() ) : while ( $wp_query->have_posts() ) : $wp_query->the_post();
    
    $topic_post_id = explode(',',get_post_meta( get_the_ID(), 'topic_post_id', true ));
    if(in_array($get_post,$topic_post_id)){
        echo '<li>';
        echo '<a href="'.admin_url('post.php').'?post='.get_the_ID().'&action=edit" target="_blank">'.get_the_title().'</a>';
        echo '</li>';
    }
...
/*当前专题下的文章*/
public function topic_post($values) {
    $topic_post_id = explode(',',get_post_meta( get_the_ID(), 'topic_post_id', true ));
    $this->before_tags($values);
    echo $values['desc'];
    echo '<ol>';
    $args=array( 'post_type'=> 'post','post__in'=> $topic_post_id,'posts_per_page' => -1,'ignore_sticky_posts' => 1 );$wp_query = new WP_Query( $args );if ( $wp_query->have_posts() ) : while ( $wp_query->have_posts() ) : $wp_query->the_post();
    
    echo '<li>';
    echo '<a href="'.admin_url('post.php').'?post='.get_the_ID().'&action=edit" target="_blank">ID:'.get_the_ID().' '.get_the_title().'</a>';
    echo '</li>';
    
    endwhile; else:
    echo __('暂时没有专题文章。','salong');
    endif;
    echo '</ol>';
    $this->after_tags();
}
...
//require __DIR__ .'/options_feild.php';
require __DIR__ .'/postmeta-feild.php';
require __DIR__ .'/termmeta-feild.php';
require __DIR__ .'/quick_edit.php';
